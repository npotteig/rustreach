    ...
    for pth in paths_vec.iter(){
        ...
        while !collision && !no_subgoal && step < total_steps && distance(&state, &final_goal_waypoint) > thresh {
            ...
            if use_subgoal_ctrl {
                ...
                let (safe, subgoal, _) = 
                if use_rtreach {
                    select_safe_subgoal_rtreach(&mut bicycle_model, state, prev_goal_waypoint, cur_goal_waypoint, num_subgoal_cands, sim_time, step_size, wall_time_ms, start_ms, store_rect, fixed_step, use_rtreach_dynamic_control)
                }
                else{
                    select_safe_subgoal_circle(&state, prev_goal_waypoint, cur_goal_waypoint, num_subgoal_cands*10)
                };
